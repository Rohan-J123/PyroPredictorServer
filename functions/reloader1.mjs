...
async function processGeoJSON(districtNumber1) {
    try {
        ...
        const districtFeature = geojsonData.features[districtNumber1 - 1];
        ...
        const previousData = await readDocument(String(districtNumber1));
        ...
        console.error(`Error processing district ${districtNumber1}:`, error.message);
    }
}

export default async (req) => {
    try {
        let { districtNumber1 } = (await readDocument("districtNumber1")) || { districtNumber1: 1 };
        await processGeoJSON(districtNumber1);
        await writeDocument("districtNumber1", { districtNumber1 });

        districtNumber1 += 5
        districtNumber1 = (districtNumber1 - 1) % 755 + 1;
