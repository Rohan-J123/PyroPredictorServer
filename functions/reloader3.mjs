...
async function processGeoJSON(districtNumber3) {
    try {
        ...
        const districtFeature = geojsonData.features[districtNumber3 - 1];
        ...
        const previousData = await readDocument(String(districtNumber3));
        ...
        console.error(`Error processing district ${districtNumber3}:`, error.message);
    }
}

export default async (req) => {
    try {
        let { districtNumber3 } = (await readDocument("districtNumber3")) || { districtNumber3: 3 };
        await processGeoJSON(districtNumber3);
        await writeDocument("districtNumber3", { districtNumber3 });

        districtNumber3 += 5
        districtNumber3 = (districtNumber3 - 1) % 755 + 1;
