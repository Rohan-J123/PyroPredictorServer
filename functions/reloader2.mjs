...
async function processGeoJSON(districtNumber2) {
    try {
        ...
        const districtFeature = geojsonData.features[districtNumber2 - 1];
        ...
        const previousData = await readDocument(String(districtNumber2));
        ...
        console.error(`Error processing district ${districtNumber2}:`, error.message);
    }
}

export default async (req) => {
    try {
        let { districtNumber2 } = (await readDocument("districtNumber2")) || { districtNumber2: 2 };
        await processGeoJSON(districtNumber2);
        await writeDocument("districtNumber2", { districtNumber2 });

        districtNumber2 += 5
        districtNumber2 = (districtNumber2 - 1) % 755 + 1;
