...
async function processGeoJSON(districtNumber4) {
    try {
        ...
        const districtFeature = geojsonData.features[districtNumber4 - 1];
        ...
        const previousData = await readDocument(String(districtNumber4));
        ...
        console.error(`Error processing district ${districtNumber4}:`, error.message);
    }
}

export default async (req) => {
    try {
        let { districtNumber4 } = (await readDocument("districtNumber4")) || { districtNumber4: 4 };
        await processGeoJSON(districtNumber4);
        await writeDocument("districtNumber4", { districtNumber4 });

        districtNumber4 += 5
        districtNumber4 = (districtNumber4 - 1) % 755 + 1;
